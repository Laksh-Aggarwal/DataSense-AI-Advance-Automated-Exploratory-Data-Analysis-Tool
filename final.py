...
# IMPORTANT: API keys are now handled in the sidebar and session state.
# GEMINI_API_KEY = "AIzaSyBrqgw5WuPDD-ht8gSnzmPVv3XuVC5nOSo"  # This is no longer used
# PPLX_API_KEY = "" # This is no longer used
...
def init_session_state():
    defaults = {
        ...
        'dashboard_col_overrides': {}, # To store column type overrides for the dashboard
        'gemini_api_key': None, # To store user-provided Gemini API key
        'pplx_api_key': None, # To store user-provided Perplexity API key
    }
...
def perplexity_chat(question, df, prompt_type="all_rounder", system_message=None):
    PPLX_API_KEY = st.session_state.get('pplx_api_key')
    if not PPLX_API_KEY:
        return "❌ Perplexity AI API Key is not set. Please enter it in the 'API Keys' section in the sidebar."
...
def chat_bot(messages, generation_config=None):
    ...
    if not API_KEY:
        return "❌ Google Gemini API Key is not set. Please enter it in the 'API Keys' section in the sidebar."
...
with st.sidebar.expander("🔑 API Keys", expanded=True):
    st.info("Enter your API keys to enable AI features.")
    
    gemini_key = st.text_input(
        "Google Gemini API Key", 
        type="password", 
        key="gemini_api_key_input",
        help="Required for Gemini models, AI Sandbox, and some explanations."
    )
    if gemini_key:
    ...
    pplx_key = st.text_input(
        ...
        type="password", 
        key="pplx_api_key_input",
        help="Required for Perplexity models, AI data cleaning, and some explanations."
    )
    if pplx_key:
        st.session_state.pplx_api_key = pplx_key
        
    if st.session_state.get('gemini_api_key') and st.session_state.get('pplx_api_key'):
        st.success("Both API keys are set!")
    elif st.session_state.get('gemini_api_key'):
        st.success("Gemini API key is set.")
        st.warning("Perplexity key is missing.")
    elif st.session_state.get('pplx_api_key'):
        st.success("Perplexity API key is set.")
        st.warning("Gemini key is missing.")
    else:
        st.warning("Please enter your API keys to use AI features.")
...
if df is not None:
    ...
    if st.session_state.get('dataset_summary') is None and st.session_state.get('pplx_api_key'):
...
def landing_page_view():
    ...
    if st.session_state.dataset_summary and st.session_state.data_metrics:
        ...
        with col1:
            with st.container():
                st.markdown("""<div class="nav-box"><h3>📊 Auto-Dashboard</h3><p>View an automatically generated dashboard of key metrics.</p></div>""", unsafe_allow_html=True)
                if st.button("View Dashboard", key="nav_dashboard", use_container_width=True):
                    st.session_state.page = 'auto_dashboard'
                    st.rerun()
        with col2:
            with st.container():
                st.markdown("""<div class="nav-box"><h3>📄 Data Profile</h3><p>View raw data, statistics, and column details.</p></div>""", unsafe_allow_html=True)
        ...
        with col4:
            with st.container():
                st.markdown("""<div class="nav-box"><h3>📈 Visualisation</h3><p>Create interactive charts to explore your data visually.</p></div>""", unsafe_allow_html=True)
                if st.button("Launch Visualisation", key="nav_viz", use_container_width=True):
                    st.session_state.page = 'visualisation'
                    st.rerun()
        with col5:
            with st.container():
                st.markdown("""<div class="nav-box"><h3>🛠️ Smart Features</h3><p>Use the Pivot Table and Hypothesis Tests for advanced analysis.</p></div>""", unsafe_allow_html=True)
                if st.button("Use Smart Features", key="nav_smart", use_container_width=True):
                    st.session_state.page = 'smart_features'
                    st.rerun()
    else:
        st.info("Waiting for data analysis to complete... Please ensure API keys are entered in the sidebar.")

def auto_dashboard_view():
    st.header("📊 Advanced Auto-Dashboard")
    st.info("This dashboard provides a comprehensive, automated analysis of your dataset. For fully custom charts, please use the 'Visualisation' tab.")

    if df is None or df.empty:
        st.warning("Please load a dataset to view the dashboard.")
        return

    # --- NEW: Column Type Override Feature ---
    with st.expander("⚙️ Advanced: Override Column Types for Dashboard"):
        st.info("Use this if a column is numeric but should be treated as categories (e.g., Year), or vice-versa. The changes only apply to this dashboard.")
        
        col_to_override = st.selectbox(
            "Select a column to treat differently:",
            ["-"] + df.columns.tolist(),
            key="dashboard_override_col_select"
        )
        ...
        if col_to_override != "-":
            ...
            override_as = st.radio(
                ...
                ("Numeric", "Categorical"),
                key=f"dashboard_override_as_{col_to_override}",
                horizontal=True
            )
            
            if st.button(f"Apply Override for '{col_to_override}'", key=f"dashboard_apply_override_{col_to_override}"):
                st.session_state.dashboard_col_overrides[col_to_override] = override_as
                st.success(f"Applied override: '{col_to_override}' will now be treated as '{override_as}'.")
                st.rerun()

        if st.session_state.dashboard_col_overrides:
            st.markdown("---")
            st.markdown("**Current Dashboard Overrides:**")
            overrides_df = pd.DataFrame(
                st.session_state.dashboard_col_overrides.items(),
                columns=['Column', 'Treated As']
            )
            st_dataframe_dual(overrides_df, hide_index=True)
            if st.button("Clear All Dashboard Overrides"):
                st.session_state.dashboard_col_overrides = {}
                st.rerun()

    # --- Identify Column Types with Overrides ---
    ...
    overrides = st.session_state.get('dashboard_col_overrides', {})
    ...
    tab_titles = ["⭐ KPIs & Overview"]
    if numeric_cols or categorical_cols:
        tab_titles.append("🔬 Single Variable Analysis")
    if len(numeric_cols) >= 2 or (len(numeric_cols) >= 1 and len(categorical_cols) >= 1):
        tab_titles.append("🔗 Relationship Analysis")
    if datetime_cols:
        tab_titles.append("🕒 Time-Series Analysis")
    ...
    with tabs[0]:
        ...
        kpi_cols[0].metric("Total Rows", f"{df.shape[0]:,}")
        kpi_cols[1].metric("Total Columns", f"{df.shape[1]:,}")
        kpi_cols[2].metric("Missing Cells", f"{df.isnull().sum().sum():,}")
        kpi_cols[3].metric("Duplicate Rows", f"{df.duplicated().sum():,}")
    ...
    if "🔬 Single Variable Analysis" in tab_titles:
        with tabs[tab_titles.index("🔬 Single Variable Analysis")]:
            ...
            if not categorical_cols:
                ...
                for col in categorical_cols:
                     with st.expander(f"Analysis of: {col}"):
                        ...
                        with c2:
                            fig_bar = px.bar(value_counts, x=value_counts.index, y=value_counts.values, title=f"Counts of {col}")
                            st.plotly_chart(fig_bar, use_container_width=True)
                            if len(value_counts) <= 10:
                                fig_pie = px.pie(names=value_counts.index, values=value_counts.values, title=f"Proportions of {col}")
                                st.plotly_chart(fig_pie, use_container_width=True)
    
    # --- Relationship Analysis Tab ---
    if "🔗 Relationship Analysis" in tab_titles:
        with tabs[tab_titles.index("🔗 Relationship Analysis")]:
            ...
            if len(numeric_cols) >= 2:
                ...
                top_pairs = corr_pairs[corr_pairs != 1.0][:5]
                st.markdown("##### Top 5 Correlated Variable Pairs")
                st.dataframe(top_pairs.to_frame(name="Correlation"))

                for (var1, var2), corr_val in top_pairs.items():
                    fig_scatter = px.scatter(df, x=var1, y=var2, title=f"{var1} vs. {var2} (Corr: {corr_val:.2f})", trendline="ols", trendline_color_override="red")
            ...
            if numeric_cols and categorical_cols:
                 key_numeric = df[numeric_cols].var().idxmax()
                 st.info(f"Showing distributions of the most variant numeric column (**{key_numeric}**) across different categories.")
                 for cat_col in categorical_cols:
                     if df[cat_col].nunique() <= 15: # Avoid plotting for high-cardinality columns
                        with st.expander(f"Analysis of '{key_numeric}' by '{cat_col}'"):
                            fig_box_cat = px.box(df, x=cat_col, y=key_numeric, title=f"Distribution of {key_numeric} across {cat_col} categories")
                            st.plotly_chart(fig_box_cat, use_container_width=True)

            else:
                st.info("Need at least one numeric and one categorical column for this analysis.")

    # --- Time-Series Analysis Tab ---
    if "🕒 Time-Series Analysis" in tab_titles:
        with tabs[tab_titles.index("🕒 Time-Series Analysis")]:
            st.subheader("Time-Series Exploration")
            time_col = st.selectbox("Select a datetime column:", datetime_cols)
            ...
            if time_col and numeric_cols:
                ...
                granularity = st.radio(
                    ...
                    ("D", "W", "M", "Q", "Y"),
                    captions=["Daily", "Weekly", "Monthly", "Quarterly", "Yearly"],
                    horizontal=True, key="time_granularity"
                )
                
                selected_num_cols = st.multiselect(
                    "Select numeric columns to plot over time:",
                    numeric_cols,
                    default=numeric_cols[:min(3, len(numeric_cols))] # Default to first 3
                )
                
                if selected_num_cols:
                    df_resampled = df_time[selected_num_cols].resample(granularity).mean()
                    fig_time = px.line(df_resampled, x=df_resampled.index, y=selected_num_cols, title=f"Time-Series of Selected Columns (Mean aggregation by {granularity})")
                    st.plotly_chart(fig_time, use_container_width=True)
                else:
                    st.warning("Please select at least one numeric column to plot.")
            else:
                st.info("A datetime column and at least one numeric column are required for time-series analysis.")
...
def ai_analysis_view():
    ...
    with main_col:
        ...
        with tab4_1:
            ...
            st.markdown("---")
            st.markdown("### ✔️ Verify a Summary")
            st.info("Paste your own summary of the data below, and the AI will verify its correctness against the loaded dataset.")

            user_summary = st.text_area(
                "Paste your summary here:",
                height=150,
                key="user_summary_to_verify"
            )

            if st.button("Verify Summary with AI", key="verify_summary_btn"):
                if user_summary:
                    with st.spinner("AI is verifying your summary..."):
                        system_prompt = (
                            "You are a meticulous data verification expert. You will be given a user's summary of a dataset and a detailed, machine-generated context of that same dataset. "
                            "Your task is to compare the user's summary against the actual data context. "
                            "Provide a point-by-point verification. For each claim in the user's summary, state whether it is correct or incorrect, and provide the correct information from the data context if it's wrong. "
                            "If the user's summary misses any crucial points (like major missing data, key outliers, or strong correlations), point them out as 'Missing Observations'. "
                            "Structure your response clearly using markdown."
                        )
                        verification_response = perplexity_chat(
                            question=f"Here is the user's summary I need you to verify:\n\n---\n\n{user_summary}\n\n---\n\nPlease verify it against the dataset context provided.",
                            df=df,
                    ...
                    st.warning("Please paste a summary into the text area before verifying.")
...
if st.session_state.get('df_original') is not None:
    ...
    st.warning("Don't forget to enter your API keys in the 'API Keys' section of the sidebar to enable all AI features.")